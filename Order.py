...
with c1:
    st.image("images/idli.png", use_container_width=True)
    idli = st.number_input("Idli - $3.00", step=1, key="idli") * 3.00
with c2:
    st.image("images/dosai.png", use_container_width=True)
...
with c1:
    st.image("images/naan.png", use_container_width=True)
    naan = st.number_input("Naan - $3.50", step=1, key="naan") * 3.50
with c2:
    st.image("images/chapati.png", use_container_width=True)
    chapati = st.number_input("Chapati - $4.30", step=1, key="chapati") * 4.30
with c3:
    st.image("images/prata.png", use_container_width=True)
    prata = st.number_input("Prata - $4.50", step=1, key="prata") * 4.50
with c4:
    st.image("images/curry.png", use_container_width=True)
...
with c1:
    st.image("images/sweets.png", use_container_width=True)
    sweets = st.number_input("Sweets - $2.00", step=1, key="sweets") * 2.00
with c2:
    st.image("images/crunchy.png", use_container_width=True)
    crunchy = st.number_input("Crunchy Snacks - $2.00", step=1, key="crunchy") * 2.00
with c3:
    st.image("images/drinks.png", use_container_width=True)
